...
[`Casks/memoryclip.rb`](Casks/memoryclip.rb) next to the source it installs, and each
release points it at that release's zip.

The trust line is Homebrew 6, which will not load a cask from a third-party tap until
you say you trust it — reasonably, since a cask is code that runs on your Mac. Skip it
and you must name the cask in full every time, `brew install --cask
yaminerl/memoryclip/memoryclip`, and a bare `brew upgrade` silently passes the tap by.
On Homebrew 5 the command does not exist and is not needed; drop that line.

Afterwards:

```sh
brew upgrade --cask memoryclip          # pick up a later release
brew uninstall --cask memoryclip        # remove the app, keep the clips
brew uninstall --zap --cask memoryclip  # remove the clip history and preferences too
```

The cask refuses to install on anything but macOS 26 or later on Apple silicon, and it
clears the quarantine flag Homebrew puts on every download — see below for why that
matters.
...
the built disk image as an attachment, plus a `.zip` of the bare `.app`.

MemoryClip is **ad-hoc signed only** — no Developer ID, no notarisation — so
Gatekeeper blocks the first launch on any Mac that did not build it. Strip the
quarantine flag:
...
That is the same command the Homebrew cask runs for you after installing, which is
the one real difference between the two routes.

Either way, it is a menu-bar agent (`LSUIElement`) with no Dock icon: look for the
clipboard glyph in the menu bar, or press **⇧⌘V** to open the panel. Prefer to build
...
It finishes by pointing the Homebrew cask at the release it just made, committing and
pushing that one-line bump, because a tap that still names the previous version keeps
installing the previous version. The checksum is taken from the asset GitHub serves
rather than the local copy in `dist/`. `SKIP_CASK=1` leaves the cask alone, and a draft
release skips it too — its assets are not downloadable yet, so run it once the draft
goes public:
...
bump it there and the app, the DMG name, the tag and the cask all follow.
...
Scripts/publish_release.sh  tag + GitHub release with the DMG attached, then the cask
Scripts/update_cask.sh  points Casks/memoryclip.rb at a published release
Casks/memoryclip.rb     the Homebrew cask; this repository is also the tap
```
